...
def loadModel():
    ...
    # model load in, must run once, to get neibu status value
...
# once file change, reload automaticaly
# Setup watchdog
observer = Observer()
observer.schedule(ReloadModelHandler(), path=MODEL_DIR, recursive=False)
observer.start()

# given a content, return a classify number
def classify(text):
    #token
    text_tokens = word_tokenize(text)
    #use stopword stemming
    ...
    #transform: get a classify number
    ...
    #convert number to class name string
    print(y_predicted[0])
